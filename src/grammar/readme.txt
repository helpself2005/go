...
1.搭建Http服务 ==
2.连接数据库 ==
sql.Row如果不遍历完或者直接调用Close()方法，执行这次查询的连接就会一直存在！当连接池里的可用连接用光后，就开始创建新的连接。这就是为什么调用SetMaxOpenConns没有用的原因，因为这个函数只是设置连接池里的连接数而已！如果因为不及时释放连接而让连接池干掉了，还是会不断的创建新的连接，直到用光MySql所有的连接，报错。明白以后，在所有调用DB.Query的函数里加上了：defer row.Close()

Ping和Exec方法在调用完之后，会自动释放连接。把代码中所有不需要返回值的语句改成由Exce方法执行，go run 一下，ok，连接数终于正常了！
3.本地IO ==
4.多线程 ==
5.网络 ==
6.调用本地命令
...
#go学习杂记#
在这个新年的假期，加入了go的学习队伍，断断续续坚持这看完了几本书，但是当我越往后看的时候，越觉得这货怎么那么像pascal；
连声明都那么像，对于一个偷懒想抄近道的人在大学选择了object pascal为基础的delphi开发工具，当时Borland的名头风头正劲，
连公司大楼的选址都那么传奇，结果却被微软的跨平台坑了爹；至今不明白Anders Hejlsberg搞了个.net的跨平台到底跨到那去了，越来越臃肿的
中年老大妈。
    一路从pascal、C#、java奔跑而来，一直喜欢delphi的编译即可用，虽然不可跨平台，老派的传统一定要手动释放；直到见到go，这小清新迎面扑来，静态类型语言却内置runtime，支持垃圾回收， defer、panic的处理手法让你不用再时刻想着最后去释放，跨平台编译不依赖系统的信息(多希望当年pascal如此)，高并发的处理和多核的应用在工业多核时代是绝对优势。

再说说它的顽固，禁止多余的引用和未使用的变量，不允许左花括号换行，没有泛型支持、不支持动态加载类库，这对于新生代的人都是缺点，但是对于老派的运动员都还是喜欢那个美如画的中距离后仰跳投，不喜欢那个超远距离三分。

最后还是要说，最适合的才是最需要的，选材还要看要做什么饭————致新东方厨师。
